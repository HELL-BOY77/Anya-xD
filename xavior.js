...
case 'stickerwm': case 'swm': case 'take': case 'sgifwm': {
               if (!quoted) throw `Replay Video/Image Dengan Caption ${prefix + command}  𝘼𝙣𝙮𝙖-𝙭𝘿`
                let [teks1,  𝘼𝙣𝙮𝙖-𝙭𝘿] = text.split`,`
                if (!teks1) throw `reply image/video With caption ${prefix + command}  𝘼𝙣𝙮𝙖-𝙭𝘿`
                if (!teks2) throw `reply image/video With caption ${prefix + command} 𝘼𝙣𝙮𝙖-𝙭𝘿`
            	m.reply(mess.wait)
                if (/image/.test(mime)) {
                    let media = await quoted.download()
                    let encmedia = await xavior.sendImageAsSticker(m.chat, media, m, { packname: teks1, author: 𝘼𝙣𝙮𝙖-𝙭𝘿})
                    await fs.unlinkSync(encmedia)
                } else if (/video/.test(mime)) {
                    if ((quoted.msg || quoted).seconds > 11) return m.reply('Maximum 10 seconds')
                    let media = await quoted.download()
                    let encmedia = await xavior.sendVideoAsSticker(m.chat, media, m, { packname: teks1, author:  𝘼𝙣𝙮𝙖-𝙭𝘿 })
                    await fs.unlinkSync(encmedia)
                } else {
                    throw `Send Image/Video With Caption ${prefix + command}\nVideo Duration 1-9 Seconds`
                }
		...
		let { webp2mp4File } = require('./lib/uploader')
            ...
            break
            case 'toaud': case 'mp3': {
            if (!/video/.test(mime) && !/audio/.test(mime)) throw `Kirim/Reply Video/Audio What You Want To Make Audio With Caption ${prefix + command}`
            if (!quoted) throw `Kirim/Reply Video/Audio What You Want To Make Audio With Captions ${prefix + command}`
            m.reply(mess.wait)
            let media = await quoted.download()
...
break
case 'sewabot':
anu = `Not Available At This Moment.`
let btn = [{
                                quickReplyButton: {
                                    displayText: '👀',
                                    id: '🌚'
                    ...
                    sections: [{
							...
							},
							{
								...
								"rows": [
									{
										"title": "Sewabot",
										"description": "Biar Bisa Masukin Ke Grup Lu :v",
										"rowId": ` sewabot`
									}
								]
							},
