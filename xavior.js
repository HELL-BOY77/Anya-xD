...
${Array.from(room.jawaban, (jawaban, index) => {
	...
	if (tebaktebakan.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            } else m.reply('*ðŸ˜Lu's Answer is Wrong Bng! *')
        }
...
*❐ hapusvote* - untuk menghapus vote`
let buttonsVote = [
  {buttonId: `❐ upvote`, buttonText: {displayText: 'ðš„ð™¿ðš…ð™¾ðšƒð™´'}, type: 1},
  {buttonId: `❐ devote`, buttonText: {displayText: 'ð™³ð™´ðš…ð™¾ðšƒð™´'}, type: 1}
]
...
break
	    ...
	    break
            ...
            case 'stickerwm': case 'swm': case 'take': case 'sgifwm': {
                ...
                if (!teks1) throw `reply image/video with caption ${prefix + command} Anya-Xd, WhatsApp-Bot`
                if (!teks2) throw `reply image/video with caption ${prefix + command} Anya-Xd, WhatsApp-Bot`
            	m.reply(mess.wait)
                if (/image/.test(mime)) {
                    let media = await quoted.download()
